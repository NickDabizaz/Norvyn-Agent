...
- **`CONTEXT-MAP.md`** at the repo root if it exists — it points at one `CONTEXT.md` per context. Read each
...
├── CONTEXT.md
├── docs/adr/
│   └── 0001-codex-owns-conversation-history.md
└── src/
```

Multi-context repos are signalled by a `CONTEXT-MAP.md` at the root, which points at one `CONTEXT.md` per
context, each with its own `docs/adr/`. That layout does not apply here.
